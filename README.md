...
> **⚠️ 关于新版微信（4.x）**
>
> 本仓库在原版基础上加了 **PC 微信 4.0 / 4.1 适配层**（`wx4/` 包）。
> 原版代码只认微信 3.x（`WeChat.exe` + `WeChatWin.dll` + `WeChat Files\<wxid>\Msg`），
> 在最新版微信上会「一直检测不到微信运行和登录」，无法解密取数。
> 4.x 的用法和原理见 **[doc/微信4.x适配说明.md](./doc/微信4.x适配说明.md)**。
>
> 快速开始（微信登录状态下）：
>
> ```bash
> pip install -r requirements.txt
> python -m wx4 info      # 先看能不能检测到微信
> python -m wx4 all       # 一键：取密钥 -> 解密 -> 导出
> python main.py          # 图形界面（自动识别 4.x / 3.x）
> ```
>
> 导出结果在 `app/DataBase/export/`（打开 `index.html` 即可浏览）。

> **🔒 隐私提醒**
>
> 运行本工具会产生一些**只属于你自己**的文件，发布 / 分享前务必清理：
>
> | 产物 | 内容 | 处理 |
> |---|---|---|
> | `wx4_keys.json` | **数据库密钥**（等同账号隐私） | 绝不要提交 / 分享 |
> | `app/DataBase/decrypted/` | 解密后的完整聊天数据库 | 不要提交 |
> | `app/DataBase/export/` | 导出的聊天记录、图片、语音 | 不要提交 |
> | `app/log/logs/` | 运行日志 | 不要提交 |
>
> 这些路径已加入 `.gitignore`。可以用本仓库自带脚本自查：
>
> ```bash
...
> **📄 二次修改声明**
>
> 本仓库是 [LC044/WeChatMsg](https://github.com/LC044/WeChatMsg) 的二次修改版本，
> 按 **GPL-3.0** 分发。新增了什么、改了哪几个文件，见
> **[doc/修改声明.md](./doc/修改声明.md)**。上游 `LICENSE` 与署名均完整保留。
...
原版流程（下面第 1~4 步）针对微信 3.x。如果你装的是 **微信 4.0 / 4.1**，
请直接用新增的适配层，完整说明见 [doc/微信4.x适配说明.md](./doc/微信4.x适配说明.md)：

```bash
python -m wx4 info                  # 检测微信进程 / 数据目录 / 账号
python -m wx4 key                   # 提取数据库密钥 -> wx4_keys.json
python -m wx4 decrypt               # 解密 -> app/DataBase/decrypted
python -m wx4 sessions              # 列出会话
python -m wx4 export -f html        # 导出 -> app/DataBase/export
python -m wx4 all                   # 上面四步一键完成
python -m wx4 all -f html --media   # 连图片/语音/视频/文件一起导出
```

图形界面：`python main.py`（自动判断 4.x / 3.x）。
